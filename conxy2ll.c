...
# zuo,Mar 26, 2023
# conxy2ll.c
# ref: https://www.cgms-info.org/wp-content/uploads/2021/10/cgms-lrit-hrit-global-specification-(v2-8-of-30-oct-2013).pdf
# ref: github.com/vensing/FY4A
# modify to C version.
----------------------------------------------------------------------------- */
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
int lc2latlon(int line,int column,int resolution,double res[]);
int latlon2lc(double lat,double lon,int resolution,double res[]);
enum{ R0500M, R1000M, R2000M, R4000M, RLEN};
float COFF[RLEN]={
	11000.5, //10991.5,
	5500.5,  //5495.5, 
	2750.5,  //2747.5, 
...
float CFAC[RLEN]={ 81865099, 40932549, 20466274, 10233137 };

#define NUM 2747
//#define NUM 47
int main(){
	int l,c;
	double storage[2],lat,lon;
	l=1660,c=1479; //差不多应该在赤道上
  // x->c->lon  //l对应于纬度
  // y->l->lat  //c对应于经度
	//	for (l = 0; l < NUM; l++){
	//		for (c = 0; c < NUM; c++){

	c=1660,l=1479; //差不多应该在赤道上
	lc2latlon(l ,c,R4000M,storage);
	printf("XY[l=%5d,c=%5d]=[lon=%9.4f,lat=%9.4f]\n",l,c,storage[0],storage[1]);
	c=1660,l=1478; //差不多应该在赤道上
	lc2latlon(l ,c,R4000M,storage);
	printf("XY[l=%5d,c=%5d]=[lon=%9.4f,lat=%9.4f]\n",l,c,storage[0],storage[1]);
	c=1550,l=57;
	lc2latlon(l ,c,R4000M,storage);
	...
	c=2060,l=1489; //差不多应该在赤道上
	lc2latlon(l ,c,R4000M,storage);
	printf("XY[l=%5d,c=%5d]=[lon=%9.4f,lat=%9.4f]\n",l,c,storage[0],storage[1]);
	lon=storage[0],lat=storage[1];
	latlon2lc(lat,lon,R4000M,storage);
	printf("LL[lon=%9.4f,lat=%9.4f]=[l=%.5f,c=%.5f]\n",lon,lat,storage[0],storage[1]);


	//lc2latlon(2530 ,1479,R4000M,storage);
	//printf("XY[%5d,%5d]=[%9.4f,%9.4f]\n",l,c,storage[0],storage[1]);
	//lon=storage[0],lat=storage[1];
	//latlon2lc(lat,lon,R4000M,storage);
	//printf("LL[%9.4f,%9.4f]=[%.5f,%.5f=]\n",lat,lon,storage[0],storage[1]);
...
//double[] lc2latlon(int line,int column,char *resolution) {
int lc2latlon(int line,int column,int resolution,double res[]) {

	double ea = 6378.137;  //地球的半长轴[km]
	double eb = 6356.7523;  //地球的短半轴[km]
	int h = 42164;  //地心到卫星质心的距离[km]
	double DeltD = (104.7)/180*PI;  // 卫星星下点所在经度
	...
	// Step1.求x,y
	double xreg = (column-COFF[resolution]) / (pow(2,-16)*CFAC[resolution]);
	double yreg = (line-COFF[resolution]) / (pow(2,-16)*CFAC[resolution]);
	//printf("%f %f \n",xreg,yreg);
	double x = toRadians(xreg);
	double y = toRadians(yreg);
	//printf("%f %f \n",x,y);
	...
	double cosy = cos(y);
	double siny = sin(y);
	double cos2y = pow(cosy, 2);
	double hcosxcosy = h * cosx * cosy;
	double cos2y_ea_eb_sin2y_2 = cos2y + pow((ea/eb*siny), 2);
	double sd = sqrt(pow(hcosxcosy, 2) - cos2y_ea_eb_sin2y_2 * (pow(h, 2)-pow(ea, 2)));
	double sn = (hcosxcosy - sd)/cos2y_ea_eb_sin2y_2;
	double s1 = h - sn * cosx * cosy;
	double s2 = sn * sin(x) * cosy;
	double s3 = -sn * siny;
	double sxy = sqrt(s1*s1+s2*s2);

	// Step3.求lon,lat
	double lon = toDegrees(atan(s2/s1)+DeltD);
	double lat = toDegrees(atan(pow(ea, 2)/pow(eb, 2)*s3/sxy)); 
	res[0]=lon;
	res[1]=lat;
	return 0;
}
...
int latlon2lc(double lat,double lon,int resolution,double res[]) {
	...
	double eb = 6356.7523;  //地球的短半轴[km]
	int h = 42164;  //地心到卫星质心的距离[km]
	double DeltD = toRadians(104.7);  // 卫星星下点所在经度


	// Step1.检查地理经纬度
	// Step2.将地理经纬度的角度表示转化为弧度表示
	double lata = toRadians(lat); //PI*lat/180; 
	double lona = toRadians(lon); //PI*lon/180;

	// Step3.将地理经纬度转化成地心经纬度 
	double eb2_ea2 = pow(eb, 2)/pow(ea, 2);
	double Delte = lona;
	double Phie = atan(eb2_ea2 * tan(lata));

	// Step4.求Re
	double re = eb / sqrt((1 - (1 - eb2_ea2) * pow(cos(Phie),2)));

	// Step5.求r1,r2,r3
	double Delte_DeltD = Delte - DeltD;
	double r1 = h - re * cos(Phie) * cos(Delte_DeltD);
	double r2 = -re * cos(Phie) * sin(Delte_DeltD);
	double r3 = re * sin(Phie);

	// Step6.求rn,x,y
	double rn = sqrt(r1*r1 + r2*r2 + r3*r3);
	double x = toDegrees(atan(-r2 / r1));
	double y = toDegrees(asin(-r3 / rn));
	// Step7.求c,l
	double c = COFF[resolution] + x*pow(2,-16)*CFAC[resolution];
	double l = COFF[resolution] + y*pow(2,-16)*CFAC[resolution];
	res[0]=l;res[1]=c;
...
/*
	 netcdf Z_SATE_C_BAWX_20211031052101_P_FY4A-_AGRI--_N_DISK_1047E_L2-_SSI-_MULT_NOM_20211031050000_20211031051459_4000M_V0001 {
dimensions:
x = 2748 ;
y = 2748 ;
o = 1 ;
...
float SSI(x, y) ;
SSI:long_name = "FY4A PGS L2 Surface Solar Irradiance" ;
SSI:standard_name = "Surface Solar Irradiance" ;
SSI:_Unsigned = "FALSE" ;
SSI:FillValue = -999.f ;
SSI:valid_range = 0.f, 1500.f ;
SSI:scale_factor = 1.f ;
SSI:add_offset = 0.f ;
SSI:units = "W/m2" ;
SSI:resolution = "4KM" ;
SSI:coordinates = "y x" ;
SSI:ancillary_variables = "DQF" ;
SSI:Description = "65535:Outer Space, -999.0:Fill Value, 65532:Sunzenith Angle greater than 70" ;
float DirSSI(x, y) ;
DirSSI:long_name = "FY4A PGS L2 Surface Direct Solar Irradiance" ;
DirSSI:standard_name = "Surface Direct Solar Irradiance" ;
DirSSI:_Unsigned = "FALSE" ;
DirSSI:FillValue = -999.f ;
DirSSI:valid_range = 0.f, 1500.f ;
DirSSI:scale_factor = 1.f ;
DirSSI:add_offset = 0.f ;
DirSSI:units = "W/m2" ;
DirSSI:resolution = "4KM" ;
DirSSI:coordinates = "y x" ;
DirSSI:ancillary_variables = "DQF" ;
DirSSI:Description = "65535:Outer Space, -999.0:Fill Value, 65532:Sunzenith Angle greater than 70" ;
float DifSSI(x, y) ;
DifSSI:long_name = "FY4A PGS L2 Surface Diffuse Solar Irradiance" ;
DifSSI:standard_name = "Surface Diffuse Solar Irradiance" ;
DifSSI:_Unsigned = "FALSE" ;
DifSSI:FillValue = -999.f ;
DifSSI:valid_range = 0.f, 1500.f ;
DifSSI:scale_factor = 1.f ;
DifSSI:add_offset = 0.f ;
DifSSI:units = "W/m2" ;
DifSSI:resolution = "4KM" ;
DifSSI:coordinates = "y x" ;
DifSSI:ancillary_variables = "DQF" ;
DifSSI:Description = "65535:Outer Space, -999.0:Fill Value, 65532:Sunzenith Angle greater than 70" ;
...
geospatial_lat_lon_extent:RegCenterLat = 65535.f ;
geospatial_lat_lon_extent:RegCenterLon = 65535.f ;
geospatial_lat_lon_extent:RegLength = 2748.f ;
geospatial_lat_lon_extent:RegWidth = 2748.f ;
...
byte DQF(x, y) ;
DQF:long_name = "CPN data quality flags" ;
...
:Project = "NOM" ;
:Conventions = "CF-1.7" ;
:Metadata_Conv = "Dataset Discovery v1.0" ;
:standard_name_vocabul = "CF Standard Name Table (v25, 05 July 2013)" ;
:Title = "FY4A AGRI L2 Surface Solar Irradiance" ;
:Summary = "Surface Solar Irradiance" ;
:platform_ID = "FY4A" ;
:instrument_type = "FY4A  Advanced Geosynchronous Radiation Image" ;
:instrument_ID = "AGRI" ;
:processing_level = "L2" ;
:date_created = "2021-10-31T05:20:35Z" ;
:production_site = "NSMC" ;
:production_environment = "UNIX" ;
:Version\ Of\ Software = "V1.0" ;
:Software\ Revision\ Date = "2017-09-12" ;
:scene_id = "Full Disk,possible values are Full Disk,the Southern HEMisphere,the Northern HEMisphere,Regional,China Regional" ;
:spatial_resolution = "4km at nadir." ;
:time_coverage_start = "2021-10-31T05:00:04.562Z" ;
:time_coverage_end = "2021-10-31T05:12:42.361Z" ;
:Data\ Quality = 0US ;
